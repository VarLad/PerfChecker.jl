function alloc_check()
    return Pkg.add(; name="Example", version="0.3.1")
end
...
function alloc_check(
    ...
    for modu in dependencies
        if isa(modu, Pair)
            for v in modu.second
                @warn "debug" dependencies modu
                deps = make_dependencies_versions(dependencies, modu)

                # @info "test deps" deps collect(Iterators.product(values(deps)...)) values(deps)

                for t in keys(deps), d in walkdir(dirname(pathof(t))), f in d[end]
                ...
                p = first(
                    if isnothing(threads)
                        addprocs(
                            1; exeflags=["--track-allocation=user", "--project=$path"]
                        )
                    else
                        addprocs(
                            1;
                            exeflags=[
                                "--track-allocation=user", "--project=$path", "-t $threads"
                            ],
                        )
                    end,
                )
                ...
                if v == "dev"
                    @eval @everywhere $p Pkg.develop(string($(modu.first)))
                else
                    @info "debug 3" modu.first string(modu.first) v
                    @eval @everywhere $p Pkg.add(name = string($(modu.first)), version = $v)
                end
                ...
                df[!, "filename: [$common$slash"] = map(
                    first ∘ splitext ∘ first ∘ splitext, specifics
                )
                ...
                if labeller == :oid
                    label = oid2string(
                        map(p -> joinpath(dirname(pathof(p)), ".."), targets)
                    )
                elseif labeller == :version
                    label = version2string(
                        map(p -> joinpath(dirname(pathof(p)), ".."), targets)
                    )
                end
                mkpath("mallocs")
